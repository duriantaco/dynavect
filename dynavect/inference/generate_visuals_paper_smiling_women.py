...
def generate_single_seed_figure(G, clip_model, device, seed=15678):
    ...
    try:
        checkpoint = torch.load("enhanced_modulator_checkpoint_v2.pt", map_location=device)
        dynavect_editor.modulator.load_state_dict(checkpoint['model_state_dict'])
        print("DynaVect (Ours) loaded.")
    except FileNotFoundError:
        print("enhanced_modulator_checkpoint_v2.pt not found")
        return
...
def generate_research_paper_figures(G, clip_model, device):
    ...
    seeds = [42, 95, 101, 683,  909,  
    1138, 1492, 1776, 2001, 2077, 3000, 4126,
    4540, 8912,
    9999, 1892, 1131, 4359, 12345, 11378, 15678]

    output_dir = "research_paper_figures"
    os.makedirs(output_dir, exist_ok=True)

    edit_configs = [
        {"name": "Age", "description": "Young -> Old", "edits": [{"neutral": "a young person", "target": "an old person", "strength": 0.7}]},
        {"name": "Age", "description": "Old -> Young", "edits": [{"neutral": "an old person", "target": "a young person", "strength": 0.8}]},
        {"name": "Hair_Color", "description": "Dark -> Blonde Hair", "edits": [{"neutral": "a woman with dark hair", "target": "a woman with dyed blonde hair", "strength": 0.9}]},
        {"name": "Combined", "description": "Smile + Age + Sunglasses", "edits": [
            {"neutral": "a face", "target": "a woman with sunglasses", "strength": 1.2},
            {"neutral": "a face", "target": "a smiling face", "strength": 0.9},
            {"neutral": "a woman with dark hair", "target": "a woman with dyed blonde hair", "strength": 0.9}
        ]}
    ]
